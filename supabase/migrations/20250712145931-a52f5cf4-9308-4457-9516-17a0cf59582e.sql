-- Insert 40 mock timers with different names and categories for the authenticated user
...
SELECT 
  t.id,
  t.user_id,
  t.created_at + (INTERVAL '1 day' * (session_num - 1)) + (INTERVAL '1 hour' * FLOOR(random() * 8 + 8)) as start_time,
  t.created_at + (INTERVAL '1 day' * (session_num - 1)) + (INTERVAL '1 hour' * FLOOR(random() * 8 + 8)) + 
    (INTERVAL '1 millisecond' * (300000 + random() * (t.elapsed_time - 300000))) as end_time,
  (300000 + random() * (t.elapsed_time - 300000))::bigint as duration_ms
FROM public.timers t
CROSS JOIN generate_series(1, 3) as session_num -- Generate 3 sessions per timer
