...
CREATE POLICY "Allow anonymous newsletter signups with validation" 
...
CREATE POLICY "Allow newsletter subscribers to view their own data" 
ON public.newsletter_subscribers 
FOR SELECT 
...
CREATE POLICY "Authenticated users can manage their subscription" 
ON public.subscribers 
FOR ALL 
USING (auth.uid() = user_id);

CREATE POLICY "Allow creating subscription for authenticated users" 
ON public.subscribers 
FOR INSERT 
...
-- Create index for performance and rate limiting
CREATE INDEX IF NOT EXISTS idx_newsletter_subscribers_email_domain ON public.newsletter_subscribers (split_part(email, '@', 2));
CREATE INDEX IF NOT EXISTS idx_newsletter_subscribers_created_at ON public.newsletter_subscribers (created_at);

-- Add comment for documentation
COMMENT ON TABLE public.newsletter_subscribers IS 'Stores anonymous newsletter signups separately from paid subscription management for security';
COMMENT ON FUNCTION public.secure_newsletter_signup IS 'Secure function for newsletter signups with rate limiting and validation';