...
const Analytics = () => {
  ...
  }, [sessions, dateRange, selectedTimer]);
  ...
  const analytics = useMemo(() => {
    ...
    const timerStats = timers.map(timer => {
      const timerSessions = filteredSessions.filter(session => session.timer_id === timer.id);
      const timerTime = timerSessions.reduce((sum, session) => sum + (session.duration_ms || 0), 0);
          ...
          <TabsContent value="timeline" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Time spent each day over the selected period
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <TabsContent value="timers" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Breakdown by individual timers
                </CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    {analytics.categoryData.map((category, index) => (
                      <div key={category.name} className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {category.sessions} sessions • {category.percentage.toFixed(1)}% of total time
                        </div>
                      </div>
                    ))}
                  </div>
