...
interface BubbleChart3DFixedProps {
...
export const BubbleChart3DFixed: React.FC<BubbleChart3DFixedProps> = ({
  sessions,
  startDate,
  endDate,
  onBubbleClick,
  ...
  // Process sessions into bubble data with fixed date range handling
  const bubbles = useBubbleDataProcessor({
    sessions,
    startDate,
    endDate,
    onError: (error) => {
      console.error('🔍 BubbleChart3DFixed - Data processing error:', error);
    ...
    if (isLoading) {
      timeoutRef.current = setTimeout(() => {
        if (mountedRef.current && isLoading) {
          console.log('🔍 BubbleChart3DFixed - 3D loading timeout, triggering fallback');
  ...
  const handleCanvasCreated = ({ gl }: any) => {
    if (mountedRef.current) {
      console.log('🔍 BubbleChart3DFixed - Canvas created successfully');
      setIsLoading(false);
  ...
  const handleCanvasError = (error: Error) => {
    if (mountedRef.current) {
      console.error('🔍 BubbleChart3DFixed - Canvas error:', error);
      setRenderError(error.message);
      setIsLoading(false);
      onError?.(error);
    }
    ...
    return (
      <div className="h-[400px] w-full bg-gradient-to-br from-slate-900 to-slate-800 rounded-lg flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <p className="text-xs text-slate-400">Filtered sessions: 0</p>
        </div>
      </div>
    );
  }
  ...
  try {
    return (
      ...
      >
        ...
        {/* Enhanced legend with date range info */}
        <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm rounded-lg p-3">
          <div className="text-white text-xs space-y-1">
            <div className="font-semibold mb-2">3D Bubble Chart - FIXED</div>
    ...
    console.error('🔍 BubbleChart3DFixed - Render error:', error);
    handleCanvasError(error as Error);
    return null;
  }
};

export default BubbleChart3DFixed;
